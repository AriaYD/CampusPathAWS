...
Prerequisites: Python 3.12, [`uv`](https://docs.astral.sh/uv/), [`bun`](https://bun.sh), `gcloud` CLI, bash ≥ 3.2 (macOS stock bash works), a Google Cloud project with the Vertex AI API enabled.

```bash
git clone <this repo> campuspath && cd campuspath
cp .env.example .env            # already points at the demo project; set GOOGLE_CLOUD_PROJECT to your own
...
- **A green deploy is not a green deploy until the new revision serves.** Piping `gcloud run deploy` through `tail` hid a non-zero exit; the new revision crashed at import — the gRPC Firestore client percent-encodes `(default)` in its routing header inside the container (not on the laptop, same versions) and the server rejects `%28default%29` — while the previous revision kept serving. Now the restore step fails open (start from seed, expose the error in `/v1/ops/agents`), the checkpoint talks to Firestore over REST where the path is unambiguous, deploy exit codes are checked, and mutating requests checkpoint inline before returning (Cloud Run throttles CPU between requests, so background writers cannot be relied on).
