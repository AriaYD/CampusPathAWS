...
# Firestore 走 REST（campuspath_state.checkpoint）：gRPC 客户端在容器里把路由头的
# "(default)" 编成 %28default%29 → 400（rev 00020–00022 实测，钉版本无效），REST 没有这个问题。
...
RUN pip install --no-cache-dir \
    ...
    "strands-agents[gemini,otel]>=1.55" "boto3>=1.40"
...
ENV CAMPUSPATH_MODEL_BACKEND=bedrock
