...
def test_runtime_control_pathway_is_gone(client, deps):
    """2026-08-04 用户裁定收口：启停通路**彻底不存在**——顶栏按钮已删，
    这里钉死后端：POST /ops/agent-runtime 连路由都没有（默认拒绝层直接
    拦下，不是 503"暂不可用"而是通路不存在）；GET 状态探测照常活着，
    状态灯不受影响。"""
